...
void makeBackup()
{
  ...
  std::string dateString = std::to_string(now->tm_year + 1900) + "-" + std::to_string(now->tm_mon + 1) + "-" + std::to_string(now->tm_mday) + "-" + std::to_string(now->tm_hour) + ":" + std::to_string(now->tm_min) + ":" + std::to_string(now->tm_sec);
...
void keepAllChanges(std::map<std::string, studentComp> &students, std::stack<alter>& stackAlter)
{
  ...
  std::ofstream alter("schedule/alter.txt");
  ...
  alter << "StudentCode,StudentName,Type,UcCode,ClassCode" << std::endl;

  while (!stackAlter.empty())
  {
    alter << stackAlter.top().studentCode << "," << stackAlter.top().studentName << "," << stackAlter.top().type << "," << stackAlter.top().ucCode << "," << stackAlter.top().classCode << std::endl;
    stackAlter.pop();
  }
